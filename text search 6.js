      ...
      function scrollToAndHighlightText(text) {
        ...
        containers.forEach((container, i) => {
          ...
          const walker = document.createTreeWalker(
            container,
            NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT, // Include elements to check for class
            {
              acceptNode: function (node) {
                if (
                  node.nodeType === 1 &&
                  node.classList.contains("sh-names")
                ) {
                  currentShNames = node; // Update the most recent .sh-names node
          ...
          let node;
          let currentShNames = null; // Initialize outside of the loop to store the most recent .sh-names element
          while ((node = walker.nextNode())) {
            if (node.nodeType === 3 && node.nodeValue.includes(text)) {
              if (currentShNames) {
                currentShNames.classList.add("show");
              }
              // Ensure nodeType is 3 for text nodes
