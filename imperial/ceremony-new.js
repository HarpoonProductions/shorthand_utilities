document.addEventListener("DOMContentLoaded", function () {
  function createCeremonyGuideLink(attempts = 0) {
    ...
    // ---------- FLOATING BARS (unchanged) ----------

    // ---------- FLOATING BARS (observe content, toggle bottom bars) ----------

    // Adjust when a section is considered "in view"
    const FLOATING_IO_OPTIONS = {
      root: null, // viewport
      threshold: 0.05, // ~5% visible
      // rootMargin: '0px 0px -20% 0px', // uncomment to bias visibility
    };

    function setupFloatingBarsByIntersection(items) {
      // Map ceremony id -> bar element (from the bottom-of-DOM wrapper sections)
      const barsById = new Map();
      document.querySelectorAll(".floating-day-bar[data-id]").forEach((bar) => {
        console.log("BAR", bar);
        const id = bar.getAttribute("data-id"); // e.g. "ceremony-1"
        if (id) barsById.set(id, bar);
      });

      // Track how many observed sections (for a given ceremony) are intersecting
      const counts = Object.create(null);
      items.forEach((item) => {
        counts[item.scrollToId] = 0;
      });

      // If you want only one bar visible at a time, set to true
      const EXCLUSIVE = false;

      const showBar = (id) => {
        console.log("SHOW BAR", id);
        const bar = barsById.get(id);
        if (!bar) return;

        if (EXCLUSIVE) {
          barsById.forEach((b, key) => {
            if (key === id) b.classList.add("visible");
      ...
      const hideBar = (id) => {
        console.log("HIDE BAR", id);
        const bar = barsById.get(id);
        if (!bar) return;
        // In EXCLUSIVE mode, we still only hide the one that lost all intersections
        bar.classList.remove("visible");
      };

      const updateForId = (id) => {
        if ((counts[id] || 0) > 0) showBar(id);
        else hideBar(id);
      };

      const io = new IntersectionObserver((entries) => {
        entries.forEach((entry) => {
          const id = entry.target.getAttribute("data-item-id"); // set below
          if (!id) return;
          counts[id] = Math.max(
            0,
            (counts[id] || 0) + (entry.isIntersecting ? 1 : -1)
          );
          updateForId(id);
        });
      }, FLOATING_IO_OPTIONS);

      // Observe the TRUE content sections per item; tag them so the observer knows which bar to toggle
      items.forEach((item) => {
        const contentSections = document.querySelectorAll(item.sections);
        contentSections.forEach((section) => {
          section.setAttribute("data-item-id", item.scrollToId); // "ceremony-1|2|3"
          io.observe(section);
        });
      });
