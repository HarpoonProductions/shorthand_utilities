document.addEventListener("DOMContentLoaded", function () {
  function createCeremonyGuideLink(attempts = 0) {
    ...
    caret.className = "menuCaret customCaret";
    */
    const caretButton = document.createElement("button");
    caretButton.type = "button";
    caretButton.className = "Navigation__button customCaret";
    ...
    ceremonyLink.appendChild(caretButton);
    ...
    items.forEach((item) => {
      ...
      link.addEventListener("click", function (e) {
        ...
        if (timeToggles.length > 0) {
          ...
          let toggleIndex = -1;
          if (item.scrollToId === "ceremony-1") {
            toggleIndex = 0; // First toggle group
          } else if (item.scrollToId === "ceremony-2") {
            toggleIndex = 1; // Second toggle group
          } else if (item.scrollToId === "ceremony-3") {
