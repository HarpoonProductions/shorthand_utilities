document.addEventListener("DOMContentLoaded", function () {
  function createCeremonyGuideLink(attempts = 0) {
    ...
    // ---------- FLOATING BARS (IO + rect fallback + triggers + logs) ----------
    ...
    // If you have a sticky header, bias the rect fallback a bit:
    const VIEW_TOP_OFFSET = 0;
    const VIEW_BOTTOM_OFFSET = 0;

    /**
     * CONFIG: global “always hide all” sections
     * These suppress every banner while intersecting.
     */
    const ALWAYS_HIDE_SELECTORS = ["#section-Ceremony-guides-4o5WGrWB2z"];

    /**
     * CONFIG: ceremony “enter triggers” that should hide the *current* banner.
     * For each ceremony id, list selectors for the other ceremonies’ top sections (or any
     * section you want to act as a switch boundary).
     *
     * Reasonable defaults below assume you can use the other “anchor” ids.
     * If your content uses different ids, just replace the selectors here.
     */
    ...
    function setupFloatingBarsByIntersection(items) {
      ...
      // ceremonyId -> HTMLElement[] (live list, kept fresh)
      const sectionsById = new Map(items.map((it) => [it.scrollToId, []]));
      const observedSet = new WeakSet();

      let suppressAll = false; // set true while any ALWAYS_HIDE element is intersecting
      ...
      const updateForId = (id, reason = "") => {
        const visible =
          !suppressAll && ((counts[id] || 0) > 0 || isVisibleByRects(id));
        if (DEBUG) {
          console.log(
      ...
      // ---- Observe real content sections (IO + rect fallback)
      ...
      // ---- Mutation observer: rebind if DOM swaps nodes
      ...
      // ---- NEW: “always hide all” triggers (suppression while intersecting)
        ...
        (entries) => {
          ...
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              // Only lift suppression if no other always-hide element is still intersecting
              // Re-check all:
              const anyStillIntersecting = entries.some(
                (e) => e.target !== entry.target && e.isIntersecting
              );
              if (!anyStillIntersecting) {
                suppressAll = false;
                changed = true;
                if (DEBUG)
                  console.log(
      ...
      // ---- NEW: per-ceremony “other section” triggers
      // When entering any trigger selector for a ceremony, hide *all* banners immediately,
      // then let normal logic show the correct one (prevents stale “current” banner).
      const ioSwitch = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (!entry.isIntersecting) return;
            const target = entry.target;
            // Find which ceremony this trigger is for
            for (const [
              ceremonyId,
              selectors,
            ] of CEREMONY_TRIGGER_SELECTORS.entries()) {
              // Does this target match any selector for this ceremony?
              const matches = selectors.some((sel) => target.matches(sel));
              if (matches) {
                if (DEBUG) {
                  console.log(
                ...
                hideAllBars(); // hide current banner immediately
                updateForId(ceremonyId, "switch-trigger"); // let correct banner show (if appropriate)
                break;
              }
            }
          });
      ...
      // Observe the configured triggers
