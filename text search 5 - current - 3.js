    ...
    containers.forEach((container) => {
      ...
      while ((node = walker.nextNode())) {
        if (node.nodeValue.includes(text)) {
          ...
          // Once replaced, need to ensure the container shows the highlights
  ...
  function scrollToMatch(matches, yOffset = -100) {
    ...
    const scroll = () => {
      if (matches[current]) {
        ...
        current = (current + 1) % matches.length; // Cycle through matches
        updateResultButtonText(current, matches.length);
      }
    };

    if (matches.length > 1) {
      // Optionally create buttons or other mechanisms to cycle through matches
      createResultButton(current, matches.length, scroll);
    }
